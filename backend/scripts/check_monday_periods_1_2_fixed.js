...
async function checkMondayPeriods() {
  try {
    ...
    if (sasini) {
      const sasiniPeriods = await Timetable.find({
        teacher: sasini._id,
        day: 'Monday',
        period: { $in: [1, 2, 3] }
      });

      console.log('\n=== Miss Udakanda Sasini - Monday Periods 1, 2, 3 ===');
      console.log(`Found: ${sasiniPeriods.length} entries`);
      sasiniPeriods.forEach(entry => {
